...
import { FlightPathsMap } from "@/components/FlightPathsMap";
...
/* ── Great-circle distance (Haversine) — still used for the mileage
   estimate shown while filling out the form and in the stats/by-year
   breakdown below. Unrelated to the map now; kept as-is. ────────────── */
...
// FlightPathsMap wants { id, origin, destination, airline?, flightNumber?,
// date?, miles? } — a small field-name adapter over FlightLeg's
// snake_case departure_date/flight_number.
...
{/* Route map — real great-circle arcs on Leaflet + CartoDB dark tiles,
    same stack as PathsMap.tsx on the Places page, in place of the old
    flat SVG equirectangular ArcMap. */}
{legs.length > 0 && <FlightPathsMap flights={mapFlights} />}
